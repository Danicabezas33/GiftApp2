...
export function Home({ onNavigate }: HomeProps) {
  ...
  const handleLoveBomb = (e: MouseEvent<HTMLButtonElement>) => {
        ...
        >
          ...
          <AnimatePresence>
            {popups.map(popup => (
              <motion.div
                key={popup.id}
                initial={{ opacity: 0, y: 20, x: "-50%", scale: 0.5, rotate: popup.rotation }}
                animate={{ opacity: 1, y: -40, x: "-50%", scale: popup.scale, rotate: popup.rotation }}
