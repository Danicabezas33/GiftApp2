...
export function NfcScannerModal({ levelId, onComplete }: { levelId: number, onComplete: (id: number) => void }) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 bg-white/95 flex flex-col items-center justify-center p-6 text-[#4A3B52] overflow-hidden"
    >
      <div className="relative w-64 h-64 md:w-80 md:h-80 flex flex-col items-center justify-center mb-10">
        ...
        <AnimatePresence mode="wait">
          {phase !== 'reveal' ? (
            ...
            >
              <Icon className="w-40 h-40 md:w-48 md:h-48 text-[#CDB4DB]/50 drop-shadow-xl" strokeWidth={1} />
            </motion.div>
          ) : (
            <motion.div
              key="reveal"
            ...
            >
              <div className="bg-gradient-to-br from-[#FFAFCC] to-[#FFC8DD] p-8 rounded-full shadow-[0_0_100px_rgba(255,139,167,0.6)]">
        ...
        <AnimatePresence>
          {phase === 'analyzing' && (
            <motion.div
              ...
              className="absolute left-[-20%] right-[-20%] h-1 bg-[#FFAFCC] z-10 shadow-[0_0_30px_rgba(255,139,167,0.8)] overflow-visible"
            >
              <div className="absolute inset-0 bg-[#FFAFCC] blur-md h-3 -mt-1 opacity-50" />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="text-center font-mono h-24 flex flex-col items-center justify-center">
        {phase === 'analyzing' && (
          <motion.p 
            ...
            className="text-[#FFAFCC] text-xl tracking-widest font-bold"
          >
        ...
        {phase === 'confirmed' && (
          <motion.p 
            ...
            className="text-[#4A3B52] text-2xl font-bold tracking-widest drop-shadow-[0_2px_5px_rgba(209,73,91,0.3)]"
          >
            [ ¡VÍNCULO CONFIRMADO! ]
          </motion.p>
        )}
        {phase === 'reveal' && (
          ...
          >
            <h2 className="text-4xl font-bold text-[#4A3B52]">
              ¡Nivel {activeGift.id} Desbloqueado!
            </h2>
            <button
              onClick={handlePlay}
              className="px-8 py-4 bg-[#FFAFCC] text-white rounded-full font-bold text-xl hover:scale-105 active:scale-95 transition-transform shadow-[0_4px_15px_rgba(255,139,167,0.5)]"
            >
              Continuar
            </button>
          </motion.div>
