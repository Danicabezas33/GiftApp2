...
export function Memories() {
  ...
  return (
    ...
    >
      <h2 className="text-5xl md:text-7xl font-script text-center text-[#880D1E] mb-6 drop-shadow-sm">Nuestros Recuerdos</h2>
      <div className="text-center mb-16">
        <p className="text-[#880D1E] font-serif text-xl font-medium drop-shadow-sm bg-white/30 inline-block px-4 py-1 rounded-full">Un viaje a través de nuestras mejores fotos.</p>
      </div>
      ...
      <div className="min-h-[400px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center p-20 text-[#DD2D4A]">
           ...
           <div className="py-12 px-8 text-center bg-white rounded-[2rem] border border-[#F49CBB]/30 shadow-xl shadow-[#F49CBB]/50 max-w-xl mx-auto">
            ...
            >
              {(memoriesData[activeTab] || []).map((item, idx) => (
                <motion.div 
                  ...
                  className="bg-white rounded-[2.5rem] overflow-hidden shadow-lg shadow-[#F49CBB]/40 border border-[#F49CBB]/30 hover:border-[#F49CBB] transition-all duration-500 cursor-pointer group"
                >
                  <div className="h-80 overflow-hidden relative">
                    <div className="absolute inset-0 bg-[#F49CBB]/10 group-hover:bg-transparent transition-colors z-10" />
                    <img 
                      src={item.images[0]} 
                  ...
                  <div className="p-8 text-center bg-[#FFF5F8]/50">
                    <p className="text-[#880D1E] font-serif font-medium text-xl leading-snug">{item.text}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
        ...
        {selectedMemory && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-white rounded-[3rem] overflow-hidden shadow-2xl shadow-[#F49CBB]/50 max-w-5xl w-full flex flex-col md:flex-row relative max-h-[90vh] border border-[#F49CBB]/30"
            ...
            >
              ...
              <div className="w-full md:w-3/5 h-[300px] md:h-auto shrink-0 relative group bg-[#F49CBB]/20 overflow-hidden">
                <AnimatePresence mode="wait">
                  <motion.div
                ...
                {selectedMemory.images.length > 1 && (
                  <>
                    <button 
                      ...
                      className="absolute left-6 top-1/2 -translate-y-1/2 bg-white/40 hover:bg-white/80 text-[#880D1E] p-3 rounded-full backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300"
                    >
                    ...
                    <button 
                      ...
                      className="absolute right-6 top-1/2 -translate-y-1/2 bg-white/40 hover:bg-white/80 text-[#880D1E] p-3 rounded-full backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300"
                    >
                    ...
                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 z-10">
                      {selectedMemory.images.map((_: any, idx: number) => (
                        <div 
                          key={idx} 
                          className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${idx === currentImageIndex ? 'bg-[#DD2D4A] scale-125 shadow-[0_2px_5px_rgba(221,45,74,0.5)]' : 'bg-white/60 shadow-sm'}`} 
                        />
                      ))}
                    </div>
                  </>
              ...
              <div className="w-full md:w-2/5 flex-1 min-h-0 overflow-y-auto bg-white">
                <div className="p-6 md:p-14 flex flex-col min-h-full h-fit">
                  <h3 className="text-2xl md:text-4xl font-serif font-bold text-[#880D1E] mb-6 md:mb-8 pr-10">{selectedMemory.text}</h3>
                  <div className="relative group">
                    <div className="absolute -left-4 md:-left-6 top-0 bottom-0 w-0.5 md:w-1 bg-[#F49CBB] rounded-full group-hover:bg-[#DD2D4A] transition-colors duration-500" />
                    <p className="text-[#880D1E]/80 text-lg md:text-xl leading-relaxed whitespace-pre-line italic font-serif">
                      "{selectedMemory.detail}"
                    </p>
                  </div>
                  <div className="mt-8 md:mt-12 flex justify-end mt-auto pt-4 opacity-40">
                    <Heart className="w-8 h-8 md:w-10 md:h-10 text-[#DD2D4A] fill-[#F49CBB]" />
                  </div>
                </div>
              </div>
            </motion.div>
