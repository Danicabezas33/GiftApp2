      ...
      {unlockedLevels.length === 0 ? (
        <div className="flex flex-col items-center justify-center min-h-[75vh] text-center px-4 w-full max-w-2xl mx-auto">
          ...
          >
            ...
            <motion.div
               ...
               className="absolute inset-[-10px] bg-[#FFC8DD]/40 rounded-full -z-20"
            />
            <div className="w-64 h-64 bg-white border border-[#FFC8DD]/30 rounded-full flex items-center justify-center relative shadow-sm">
               <motion.div
                  animate={{ rotate: 360 }}
                  transition={{ repeat: Infinity, duration: 30, ease: "linear" }}
                  className="absolute inset-4 rounded-full border border-dashed border-[#FFAFCC]/40"
               />
               <RadioReceiver className="w-24 h-24 text-[#FFAFCC]/80" strokeWidth={1} />
            </div>
          </motion.div>
          ...
          >
            <h2 className="text-4xl md:text-7xl font-script text-[#4A3B52] mb-6 drop-shadow-sm">
              Tu regalo te espera
            </h2>
            <p className="text-lg md:text-2xl text-[#CDB4DB] max-w-lg mx-auto font-serif leading-relaxed mb-12">
              Acerca el primer objeto al escáner NFC para comenzar el viaje...
            </p>
            
            <div className="inline-flex items-center gap-5 bg-white shadow-xl shadow-[#FFC8DD]/50 py-3 px-8 md:py-4 md:px-10 rounded-full border border-[#FFC8DD]/30">
               <div className="flex space-x-1.5">
                  <motion.div animate={{ opacity: [0.2, 1, 0.2], scale: [0.8, 1.2, 0.8] }} transition={{ repeat: Infinity, duration: 2, times: [0, 0.5, 1] }} className="w-2 h-2 bg-[#FFAFCC] rounded-full" />
                  <motion.div animate={{ opacity: [0.2, 1, 0.2], scale: [0.8, 1.2, 0.8] }} transition={{ repeat: Infinity, duration: 2, delay: 0.4, times: [0, 0.5, 1] }} className="w-2 h-2 bg-[#FFAFCC] rounded-full" />
                  <motion.div animate={{ opacity: [0.2, 1, 0.2], scale: [0.8, 1.2, 0.8] }} transition={{ repeat: Infinity, duration: 2, delay: 0.8, times: [0, 0.5, 1] }} className="w-2 h-2 bg-[#FFAFCC] rounded-full" />
               </div>
               <span className="text-sm font-mono font-bold text-[#FFAFCC] uppercase tracking-[0.3em]">Buscando...</span>
            </div>
          </motion.div>
        </div>
      ) : (
        <>
          <div className="text-center mb-10 md:mb-16">
            <h2 className="text-4xl md:text-7xl font-script text-[#4A3B52] mb-6 drop-shadow-sm">Disfruta de los regalos</h2>
            
            <div className="flex items-center justify-center gap-4 bg-white shadow-xl shadow-[#FFC8DD]/50 py-3 px-8 rounded-full max-w-sm mx-auto border border-[#FFC8DD]/30 mt-8">
              <motion.div
                animate={{ scale: [1, 1.2, 1], opacity: [0.4, 0.8, 0.4] }}
                transition={{ repeat: Infinity, duration: 3 }}
              >
                <RadioReceiver className="w-5 h-5 text-[#FFAFCC]" />
              </motion.div>
              <span className="font-mono text-xs tracking-[0.2em] uppercase text-[#CDB4DB]">Esperando conexión física...</span>
            </div>
          </div>
              ...
              return (
                <motion.div
                  ...
                  className={`relative overflow-hidden rounded-[2.5rem] p-10 transition-all duration-500 md:min-h-[280px] flex flex-col items-center justify-center text-center border group
                    ${isCompleted 
                      ? 'bg-white hover:border-[#FFC8DD] cursor-pointer shadow-xl shadow-[#FFC8DD]/50 border-[#FFC8DD]/30' 
                      : isActive
                        ? 'bg-[#FFAFCC] text-white border-[#FFAFCC] shadow-[0_10px_30px_rgba(255,139,167,0.5)] animate-[bounce_3s_infinite] cursor-default'
                ...
                >
                  ...
                  {isCompleted && (
                    <div className="absolute top-6 right-6 text-[#FFAFCC]/40 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 transition-all duration-500 relative overflow-hidden ${
                    isCompleted 
                      ? 'bg-[#FFF0F5] text-[#FFAFCC] group-hover:bg-[#FFAFCC] group-hover:text-white shadow-inner group-hover:shadow-[0_0_30px_rgba(255,139,167,0.4)]' 
                      : isActive
                  ...
                  <h3 className={`text-2xl font-serif font-bold mb-2 ${
                    isCompleted ? 'text-[#4A3B52]' : isActive ? 'text-white' : 'text-slate-400'
                  }`}>
                    Nivel {gift.id}
                  </h3>
                  <p className={`font-medium tracking-wide ${
                    isCompleted ? 'text-[#FFAFCC]' : isActive ? 'text-white/80' : 'text-slate-400/70'
                  }`}>
                  ...
                  {isCompleted && (
                    <motion.div 
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="mt-4 px-4 py-1 bg-[#FFAFCC]/10 border border-[#FFAFCC]/20 rounded-full text-[10px] text-[#FFAFCC] font-bold uppercase tracking-widest"
                    >
                      Completado
                    </motion.div>
                  )}
      ...
      {unlockedLevels.length > 0 && (
        <div className="mt-20 flex flex-col sm:flex-row items-center justify-center gap-8">
          <button
            ...
            className="text-[#CDB4DB] hover:text-[#FFAFCC] text-xs font-mono tracking-widest uppercase transition-all duration-300"
          >
            Resetear Progreso
          </button>
          <button
        ...
        {modalPhase === 'web_unlocked' && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-white p-12 md:p-16 rounded-[3rem] shadow-2xl shadow-[#FFC8DD]/50 max-w-lg w-full relative overflow-hidden border border-[#FFC8DD]/30"
            >
              <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-[#FFAFCC] to-[#4A3B52]" />
              <Globe className="w-24 h-24 text-[#FFAFCC] mx-auto mb-8 drop-shadow-[0_2px_10px_rgba(255,139,167,0.4)]" />
              <h2 className="text-4xl md:text-5xl font-script font-bold mb-6 text-[#4A3B52]">
                ¡Página web desbloqueada!
              </h2>
              <p className="text-xl text-[#CDB4DB] mb-10 font-serif leading-relaxed">
              ...
              <button
                ...
                className="w-full py-5 bg-[#FFAFCC] text-white rounded-2xl font-bold text-xl hover:scale-[1.02] active:scale-95 transition-all shadow-[0_4px_15px_rgba(255,139,167,0.4)]"
              >
        ...
        {revealedGift && activeGift && modalPhase === 'minigame' && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-white rounded-[2.5rem] overflow-y-auto shadow-2xl shadow-[#FFC8DD]/50 max-w-lg w-full max-h-[95vh] relative border border-[#FFC8DD]/30 text-center flex flex-col"
            ...
            >
              <button 
                ...
                className="absolute top-6 right-6 text-[#CDB4DB] hover:text-[#4A3B52] transition-colors z-20"
              >
                <X className="w-6 h-6" />
              </button>

              <div className="bg-[#FFAFCC]/10 py-12 px-6 relative overflow-hidden border-b border-[#FFC8DD]/30">
                ...
                <activeGift.icon className="w-20 h-20 text-[#FFAFCC] mx-auto relative z-10 drop-shadow-sm" />
              </div>

              <div className="p-8 md:p-12 flex flex-col flex-1">
                <span className="inline-block px-4 py-1 bg-[#FFAFCC] text-white text-[10px] font-bold tracking-[0.3em] uppercase rounded-full mb-6 mx-auto">
                  Misión Desbloqueada
                </span>
                <h3 className="text-3xl font-serif font-bold text-[#4A3B52] mb-6">Mini-Desafío {activeGift.id}</h3>
                ...
                <button
                  ...
                  className="w-full py-5 border border-[#FFC8DD]/50 hover:bg-[#FFF0F5] text-[#CDB4DB] hover:text-[#4A3B52] rounded-2xl font-bold text-lg transition-all duration-500"
                >
                  Cancelar desafío
                </button>
              </div>
