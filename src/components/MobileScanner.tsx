...
export function MobileScanner({ id }: { id: number }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#FFF5F8] flex flex-col items-center justify-center text-[#4A3B52] p-6">
      <motion.div 
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        className="flex flex-col items-center justify-center text-center max-w-sm w-full bg-white p-8 rounded-[2rem] shadow-xl shadow-[#FFC8DD]/50 border border-[#FFC8DD]/30"
      >
        {status === 'sending' && (
          <>
            <Loader2 className="w-16 h-16 text-[#FFAFCC] animate-spin mb-6" />
            <h2 className="text-2xl font-bold mb-2 font-serif text-[#4A3B52]">Enviando señal...</h2>
            <p className="text-[#CDB4DB]">Estableciendo vínculo con la tablet</p>
          </>
        )}
        ...
        {status === 'success' && (
          <>
            ...
            >
              <CheckCircle2 className="w-20 h-20 text-[#FFAFCC] mb-6 drop-shadow-md" />
            </motion.div>
            <h2 className="text-3xl font-bold mb-4 font-serif text-[#4A3B52]">
              ✨ ¡Señal enviada!
            </h2>
            <p className="text-[#CDB4DB] text-lg mb-8">
              Mira la tablet para continuar
            </p>
            <div className="w-full h-1 bg-[#FFF0F5] rounded-full overflow-hidden">
               <motion.div 
                 initial={{ width: 0 }}
                 animate={{ width: "100%" }}
        ...
        {status === 'invalid' && (
          <>
            ...
            <p className="text-[#CDB4DB] text-lg mb-2">
              Este no es el regalo que toca ahora mismo.
            </p>
            <p className="text-[#4A3B52]/60 text-sm">Escanea el objeto correcto para continuar.</p>
          </>
        )}
        ...
        {status === 'error' && (
          <>
            ...
            <p className="text-[#CDB4DB] mb-2">Por favor, inténtalo de nuevo.</p>
