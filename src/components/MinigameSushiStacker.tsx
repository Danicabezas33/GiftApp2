...
export function MinigameSushiStacker({ onWin }: MinigameSushiStackerProps) {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="w-full relative bg-[#FFC8DD]/50 rounded-2xl overflow-hidden border border-[#FFC8DD]/50 shadow-inner min-h-[350px] flex items-center justify-center cursor-pointer">
        <canvas 
          ref={canvasRef} 
        ...
        {!isPlaying && score === 0 && !isGameOver && !isWon && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm z-10 pointer-events-none">
            <h3 className="font-serif text-2xl text-[#4A3B52] mb-2 font-bold drop-shadow-sm">Sushi Tower</h3>
            <p className="font-sans text-[#CDB4DB] mb-6 px-6 text-center max-w-sm text-sm">
            ...
            <button 
              ...
              className="px-8 py-3 bg-[#FFAFCC] text-white font-bold rounded-full hover:bg-[#FFAFCC] transition-colors shadow-md pointer-events-auto transform hover:scale-105"
            >
              ¡A Cocinar!
            </button>
          </div>
        ...
        {isGameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/90 backdrop-blur-sm z-10 pointer-events-none">
             ...
             <p className="font-serif text-2xl text-[#4A3B52] font-bold mb-2">¡Desastre en la cocina!</p>
             <p className="text-[#CDB4DB] mb-6 font-medium text-sm">Lograste {score} pisos de sabor.</p>
             <button 
              onClick={(e) => { e.stopPropagation(); startGame(); }}
              className="px-8 py-3 bg-[#FFAFCC] text-white font-bold rounded-full hover:bg-[#FFAFCC] transition-colors shadow-md pointer-events-auto transform hover:scale-105"
            >
        ...
        {isWon && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-20">
             <div className="bg-white/90 backdrop-blur-md px-8 py-6 rounded-2xl shadow-xl shadow-[#FFC8DD]/50 border border-[#FFC8DD]/50 text-center animate-bounce mt-32">
                <p className="font-serif text-3xl text-[#4A3B52] font-bold mb-2">¡Torre Perfecta!</p>
                <p className="text-[#FFAFCC] font-medium">Cena Sushi superada 🎉</p>
             </div>
          </div>
        )}

        {isPlaying && !isWon && (
          <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-4 py-2 rounded-full text-[#4A3B52] font-bold text-sm shadow-sm z-10 border border-[#FFC8DD]/30 flex items-center gap-2 pointer-events-none">
